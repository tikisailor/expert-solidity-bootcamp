...
import { MyCon } from "src/ValidSolidity.sol";
...
contract FooTest is PRBTest, StdCheats {
    ...
    function testFoo() external {
        Foo myContract = new Foo();

        uint256 id = myContract.id(5);
        assertEq(id, 5);
    }

    function testValidSolidity() external {
        MyCon myCon = new MyCon();

        vm.expectCall(
            address(myCon), abi.encodeCall(myCon.a, myCon.b)
        );
        myCon.a(myCon.b);
    }
}
